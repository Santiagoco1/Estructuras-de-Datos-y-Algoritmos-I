...
/**
 * avl_eliminar: Elimina un dato en el arbol, manteniendo la
 * propiedad de los arboles AVL.
 */
...
static AVL_Nodo* avl_nodo_eliminar(AVL_Nodo* raiz, void* dato,
  FuncionCopiadora copia, FuncionComparadora comp) {
    if (comp(raiz->dato, dato) < 0) {
      avl_nodo_eliminar(raiz->izq, dato, copia, comp)
    } else if (comp(raiz->dato, dato) > 0) {
      avl_nodo_eliminar(raiz->der, dato, copia, comp)
    } else {
      
    }
}
/**
 * avl_eliminar: Elimina un nodo, segun el dato pasado
*/
void avl_eliminar(AVL arbol, void* dato) {
  arbol->raiz = avl_nodo_eliminar(arbol->raiz, dato, arbol->copia, arbol->comp)
}
